...
static D : usize = 30000000;
static EXPONENT : u32 = 24;
...
// Generates openings to calculate A_r[alpha] value
fn get_A_openings(domain: GeneralEvaluationDomain::<Fr>, time_ck: &CommitterKey::<Bls12_381>, time_vk: &VerifierKey::<Bls12_381>, alpha: Fr, etas: &[Fr]) 
-> (Vec<EvaluationProof<Bls12_381>>, Vec<Vec<Commitment<Bls12_381>>>, Vec<Vec<Vec<Fr>>>) {
    ...
    return (proofs, new_commitments, all_evals);
}
...
// ===================================================================================================================================
    ...
    let hashes = get_poseidon_hash_of_commmitments(coms2.clone(), 19);
    let alpha = hashes[0];
    let etas = &hashes[3..11];

    last = print_time_since(start, last, "PROVER PART 1 TOOK");     

    // Get openings to calculate A_r[alpha] value
    let (a_proofs, a_commitments, a_evals) = get_A_openings(domain, &time_ck, &time_vk, alpha, etas);
    last = print_time_since(start, last, "PROVER PART 2 TOOK");   
...
// ===================================================================================================================================
    ...
    // Re-check all A row vector commitments
    for i in 0..a_proofs.len() {
        let verification_result = time_vk.verify_multi_points(
            &a_commitments[i],
            &eval_points,
            &a_evals[i],
    ...
    // Calculate A_r[alpha] vectors
    let mut a_r_alpha = Fr::from(0);
    for i in 0..hash_coeffs.len() {
        a_r_alpha += hash_coeffs[i] * a_evals[i /  BATCH_SIZE][i % BATCH_SIZE][0];
    }
    ...
    assert!((evals2[1][1] - evals2[1][0] - evals[3][0] * a_r_alpha) * n_1_of_alpha  == evals2[2][0] * domain.evaluate_vanishing_polynomial(alpha));
