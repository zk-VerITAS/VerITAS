...
fn prove(start: u128, old_last: u128, rate_bits: usize, cap_height: usize, omega: F, fft_root_table: &FftRootTable<F>) 
-> (FriOpenings<F, D>, [MerkleCap<GoldilocksField, PoseidonHash>; 3], FriProof<F, <PoseidonGoldilocksConfig as GenericConfig<D>>::Hasher, D>, u128)  {
    ...
    last = print_time_since(start, last, "eval commitments done"); 
...
fn main() {
    ...
    // Prover
    let (fri_openings, merkle_caps, opening_proof, mut last) = prove(start, start, rate_bits, cap_height, omega, &fft_root_table);
    last = print_time_since(start, last, "proving done"); 
    ...
    let instance = get_instance(3, 7, 3, degree_bits, zeta);
    ...
    let a = PolynomialValues::from(a_vals).ifft(); 
    let a_zeta_eval = a.to_extension::<D>().eval(zeta);
    ...
    assert!((fri_openings.batches[1].values[11] - fri_openings.batches[0].values[11] - fri_openings.batches[0].values[1] * a_zeta_eval) * n_1_zeta_eval  ==  fri_openings.batches[0].values[12] * vanishing_poly_zeta_eval);
