...
// 100000 = 16.762117 minutes
// 50000 5.72825 minutes



pub struct AbsorbCircuit<F: PrimeField> {
    pub input: Vec<F>,
    pub coeffs: Vec<F>,
    pub hash: F,
    pub linear_combo_hash: F
}

/// Constructor for AbsorbCircuit
#[allow(dead_code)]
impl<F: PrimeField> AbsorbCircuit<F> {
    pub fn new(input: Vec<F>, coeffs: Vec<F>, hash: F, linear_combo_hash: F) -> Self {
        Self { input, coeffs, hash, linear_combo_hash}
    }
}

impl<F: PrimeField> Clone for AbsorbCircuit<F> {
    fn clone(&self) -> Self {
        AbsorbCircuit {
            input: self.input.clone(),
            coeffs: self.coeffs.clone(),
            hash: self.hash,
            linear_combo_hash: self.linear_combo_hash
        }
    }
}
impl<F: PrimeField, > ConstraintSynthesizer<F> for AbsorbCircuit<F> {
    fn generate_constraints(self, cs: ConstraintSystemRef<F>) -> Result<(), SynthesisError> {
        let mut input = Vec::new();

        for elem in self.input.iter() {
            input.push(FpVar::new_witness(cs.clone(), || Ok(elem))?);
        }

        let mut coeffs = Vec::new();

        for elem in self.coeffs.iter() {
            coeffs.push(FpVar::new_input(cs.clone(), || Ok(elem))?);
        }

        let res_target = FpVar::<F>::new_input(cs.clone(), || Ok(&self.hash))?;
        let mut sponge = PoseidonSpongeVar::new(cs.clone(), &poseidon_parameters_for_test());
        sponge.absorb(&input)?;
        let res_var = sponge.squeeze_field_elements(1)?;

        // check hash
        res_var[0].enforce_equal(&res_target)?;

        // check random linear combination
        let mut zero = FpVar::<F>::zero();
        for i in 0..coeffs.len() {
            zero += input[i].clone() * coeffs[i].clone();
        }
        let linear_combo_hash = FpVar::<F>::new_input(cs.clone(), || Ok(&self.linear_combo_hash))?;
        linear_combo_hash.enforce_equal(&zero)?;
...
/*impl<F: PrimeField, > ConstraintSynthesizer<F> for AbsorbCircuit<F> {
    fn generate_constraints(self, cs: ConstraintSystemRef<F>) -> Result<(), SynthesisError> {
        ...
        for elem in self.a.iter() {
            a.push(FpVar::new_witness(cs.clone(), || Ok(elem))?);
        }

        let mut coeffs = Vec::new();

        for elem in self.coeffs.iter() {
            coeffs.push(FpVar::new_witness(cs.clone(), || Ok(elem))?);
        }
...
fn circuit() {
    ...
    let absorb: Vec<_> = (0..LENGTH).map(|_| Fr::rand(rng)).collect();

    let coeffs: Vec<_> = (0..LENGTH).map(|_| Fr::rand(rng)).collect();

    let mut sum = Fr::from(0);
    for i in 0..coeffs.len() {
        sum += absorb[i] * coeffs[i];
    }
...
/*
    let (pk, vk) = {
        let c = AbsorbCircuit::<Fr>::new(absorb.clone(), hash[0], coeffs.clone(), sum);
    ...
    let c = AbsorbCircuit::<Fr>::new(absorb, hash[0], coeffs, sum);

    let proof = Groth16::<Bls12_381>::prove(&pk, c, &mut rng).unwrap();

    let res = Groth16::<Bls12_381>::verify(&vk, &vec![hash[0]], &proof).unwrap();
    assert!(res);

    assert!(
        Groth16::<Bls12_381>::verify_with_processed_vk(&pvk, &[hash[0]], &proof).unwrap()
    ); */
...
fn main() {
    ...
    let mut rng = test_rng();
        ...
        let squeeze1 = native_sponge.squeeze_native_field_elements(128);
        let squeeze2 = constraint_sponge.squeeze_field_elements(128).unwrap();
    ...
    println!("minutes since start {:?}", (now - start) as f32 / 60000.0);
